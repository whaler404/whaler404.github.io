...
$$
(w^*,b^*)=\argmin\limits_{(w,b)}\sum_{i=1}^m (y_i-wx_i-b)^2
$$
...
$$
w^*=\argmin\limits_{w}(y-w^TX)^T(y-w^TX)
$$
...
$$
p_1(\hat{x_i};\beta)=p(y=1|\hat{x};\beta)
=\frac{e^{\beta^T\hat{x}}}{1+e^{\beta^T\hat{x}}} \\
p_0(\hat{x_i};\beta)=p(y=0|\hat{x};\beta)
=1-p_1(\hat{x_i};\beta)
=\frac{1}{1+e^{\beta^T\hat{x}}} \\
p(y_i|x_i;w,b)=y_ip_1(\hat{x_i};\beta)+
...
$$
\max \ell(w,b)=\sum\limits_{i=1}^{m}\ln p(y_i|x_i;w,b)
\Longleftrightarrow \\
