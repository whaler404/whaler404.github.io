...
$$
h^*(x)=\argmin\limits_{c\in y}R(c\vert x)
$$
...
$$
h^*(x)=\argmax\limits_{c\in y}P(c\vert x)
$$
...
$$
h_{nb}(x)=\argmax\limits_{c\in y}P(c)\prod\limits_{i=1}^d P(x_i\vert c)
$$
...
$$
P(x_i\vert c)=\frac{\vert D_{c,x_i}\vert}{D}
$$
...
\hat{P}(x_i\vert c)=\frac{\vert D_{c,x_i}\vert+1}{\vert D\vert+N_i}
$$
