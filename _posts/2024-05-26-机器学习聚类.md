...
对数据集合 $D=\{x_1,\dots,x_m\}$ ，假定通过聚类划分得到的簇划分为 $C=\{C_1,\dots,C_k\}$ 参考模型给出的簇划分为 $C^*=\{C_1^*,\dots,C_s^*\}$ ，相应地，令 $\lambda,\lambda^*$ 分别表示 $C,C^*$ 对应的簇标记向量
...
> 省流：聚类需要距离度量，原型聚类分为k均值、学习向量化、高斯混合模型，密度聚类有DBSCAN
