...
\end{aligned}
\right.\\
\argmax\limits_{y\in\{-1,1\}}P(f(x)=y\vert x)
$$
...
\begin{aligned}
h_t(x)
&=\argmin\limits_h\mathcal{l}_{exp}(H_{t-1}+h_t\vert \mathcal{D})\\
&=\argmin\limits_h\mathbb{E}_{x\sim\mathcal{D}}[e^{-f(x)H_{t-1}}(1-f(x)h_t(x)+\frac{1}{2})]\\
&=\argmax\limits_h\mathbb{E}_{x\sim\mathcal{D}}[e^{-f(x)H_{t-1}}f(x)h(x)]\\
&=\argmax\limits_h\mathbb{E}_{x\sim\mathcal{D}}[\frac{e^{-f(x)H_{t-1}}}{\mathbb{E}_{x\sim\mathcal{D}}[e^{-f(x)H_{t-1}(x)}]}f(x)h(x)]
\end{aligned}
$$

注意到 $\mathbb{E}_{x\sim\mathcal{D}_t}[e^{-f(x)H_{t-1}(x)}]$ 是一个常数，令 $\mathcal{D}_t$ 表示一个分布：
...
h_t(x)
&=\argmax\limits_h\mathbb{E}_{x\sim\mathcal{D}}[\frac{e^{-f(x)H_{t-1}}}{\mathbb{E}_{x\sim\mathcal{D}}[e^{-f(x)H_{t-1}(x)}]}f(x)h(x)]\\
&=\argmax\limits_h\mathbb{E}_{x\sim\mathcal{D}_t}[f(x)h(x)]
\end{aligned}
$$
...
$$
h_t(x)=\argmin\limits_h\mathbb{x\sim\mathcal{D}_t}[\Pi(f(x)\ne h(x))]
$$
...
$$
H^{oob}(x)=\argmax\limits_{y\in\mathcal{Y}}\sum\limits_{t=1}^T\Pi(h_t(x)=y)\cdot(x\notin D_t)
$$
...
$$
H(x)=c_{\argmax\limits_{j}\sum\limits_{i=1}^Th_i^j(x)}
$$

加权投票法（weighted voting）

$$
H(x)=c_{\argmax\limits_{j}\sum\limits_{i=1}^Tw_ih_i^j(x)}
$$
