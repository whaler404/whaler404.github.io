...
![image-20230615111427996](/assets/images/cs_network.assets/image-20230615111427996.png)
...
- **调制机制使用信号来传输比特**
  ...
  - Baseband Transmission（**基带**传输）：直接将**数据比特**转化为信号
    ...
    - 比特流=>不归零NRZ（1是1，0是0）=>不归零逆转NRZI（1延续一半再跳变逆转）=>曼切斯特编码（比特流和时钟信号XOR，1是下降沿，0是上升沿）=>双极性编码（1要么高电平要么低电平，0没有电平）
      ...
      ![image-20230704134209298](/assets/images/cs_network.assets/image-20230704134209298.png)
...
- **复用技术是让多用户共享同一根信道**
  ...
  - 码分多路复用
    ...
    ![image-20230615113131533](/assets/images/cs_network.assets/image-20230615113131533.png)
...
- **将原始的位流分散到离散的帧中**，叫成帧，成帧的⽅法有：
  ...
  - **字符计数法**
    ...
    ![image-20230614161503142](/assets/images/cs_network.assets/image-20230614161503142.png){:height="50%" width="50%"}
...
![image-20230614165012559](/assets/images/cs_network.assets/image-20230614165012559.png)
...
- 计算1 0 0 1 0 0 0，先扩充0 0 1 0 0 0 1 0 0 0 0
...
![image-20230614172012700](/assets/images/cs_network.assets/image-20230614172012700.png)
...
![image-20230615100149530](/assets/images/cs_network.assets/image-20230615100149530.png)
...
- **静态分配**
  ...
  - 信道平均延迟时间
    ...
    ![image-20230613174435393](/assets/images/cs_network.assets/image-20230613174435393.png)
...
- **动态分配**
  ...
  - 基本概念（五个关键假设， P201 ）
    ...
    ![image-20230613175227774](/assets/images/cs_network.assets/image-20230613175227774.png)
  ...
  - 典型的随机访问协议
    ...
    - **分隙ALOHA协议**
      ...
      - 分隙ALOHA的性能分析
        ...
        ![image-20230613194907772](/assets/images/cs_network.assets/image-20230613194907772.png)
...
- 受控访问协议（Controlled Access）
  ...
  - **二进制倒计数协议**
    ...
    - 基本思想
      ...
      ![image-20230613203205529](/assets/images/cs_network.assets/image-20230613203205529.png)
...
![image-20230613205412173](/assets/images/cs_network.assets/image-20230613205412173.png)
...
（3）IEEE802.3Q在发布的新帧格式中包含了一个VLAN标签（tag）
...
![image-20230612175131829](/assets/images/cs_network.assets/image-20230612175131829.png)
...
> 定义最大路径、限制成环、主动断开快速收敛（毒性逆转）、定时器、触发刷新
...
- OSPF是一种基于开放标准的**链路状态路由协议**，是目前IGP中**应用最广、性能最优**的一个协议 
...
![image-20230612201735233](/assets/images/cs_network.assets/image-20230612201735233.png)
...
- 建立路由器毗邻关系
  ...
  ![image-20230612202342787](/assets/images/cs_network.assets/image-20230612202342787.png)
...
![image-20230612204549859](/assets/images/cs_network.assets/image-20230612204549859.png)
...
- 算法
  ...
  - #### 漏桶（leaky bucket）
    ...
    ![image-20230613114639607](/assets/images/cs_network.assets/image-20230613114639607.png)
    ...
    - 算法描述
      ...
      - 每个时钟嘀嗒( **tick** )，仅允许一个分组或固定数量的分组发送出去
        ...
        ![image-20230612214432272](/assets/images/cs_network.assets/image-20230612214432272.png)
  ...
  - #### 令牌桶（token bucket）
    ...
    ![image-20230613130031916](/assets/images/cs_network.assets/image-20230613130031916.png)
    ...
    - > 计算最大突发时间
      ...
      ![image-20230613131015306](/assets/images/cs_network.assets/image-20230613131015306.png)
...
- 两级路由算法
  ...
  - 每个网络内部采用**内部网关协议（IGP**， interior gateway protocol ）

  - 网络之间使用**外部网关协议（BGP**， exterior gateway protocol ）
...
分段（Fragmentation）是将一个**分组切分成几个小的分组**，已通过网络

- 透明分段
  - 分段行为对其他网络来说是**不可见的**，换句话说，在该网络分段的分组，在离开这个网络的时候需要将它**重组恢复**
  - 遇到的问题
...
- 一个互联网分组可能包含多个分片，所以，在分片的头部必须提供这些信息：
  ...
  - 一个位（bit），表明该分片是否是原始分组分割后的最后一个分片
    ...
    > 来自那个分组、第几号分片、是否是最后一片

![image-20230613133712665](/assets/images/cs_network.assets/image-20230613133712665.png)
...
- 在网络层上，可将整个互联网看作一组互联的**子网络和自治系统**
- 将整个互联网粘合起来的正是网络层协议：IP (Internet Protocol).
- IP的任务是提供一种**尽力传送（ Best-Efforts ）**的方法，将数据报从源传送到目的
...
- 所依赖的地址结构不同，MAC是平面地址，IP是**结构化、层次化地址**，其本身携带了位置信息；
- 所处的OSI模型层数不同；
...
![image-20230613140820127](/assets/images/cs_network.assets/image-20230613140820127.png)
...
- 报头长度
  - 4 bits.
  - IP 头长度：以32bit为**单位**标明报文长度 (**4 bytes)**，可指向**数据开始处**（理论报头最长为64字节，其实是60字节）
...
- 数据报总长度
  - **16 bits.（IP长度最长为65535字节）**
  - 指明整个数据分组的总长度，包括**数据头和数据**。单位“**字节**”

- 数据报标识号
  - 16 bits.
  - 标识当前数据报的**序列号**
  - 由发送者分配，以便接收方可以依据来做**重组**
...
- **分片偏移**
  ...
  - **数据封装提出的问题**
    ...
    - 数据报长度的限制
      ...
      - 硬件限制
        - 物理网络对**帧**的最大字节数限制，由硬件决定，称为最大传输单元（**MTU**）。
  ...
  - **适应不同MTU的解决方案**
    ...
    - 定义分片机制
      ...
      ![image-20230613142736731](/assets/images/cs_network.assets/image-20230613142736731.png)
...
- **生存时间**
  ...
  - **Time-to-Live(TTL)维护一个计数器**，递减为零时，数据报被丢弃，防止分组在网络中**无限循环**
...
![image-20230613145400070](/assets/images/cs_network.assets/image-20230613145400070.png)

**IP地址的分离**

![image-20230613145523234](/assets/images/cs_network.assets/image-20230613145523234.png)

> 主机数指的是一个网络中的主机数

- A类地址
  - **前1字节**标识网络地址部分，后3字节标识主机地址部分

  - **每个网络**最多可容纳 （2^24  -2）台主机

  - 第1字节用十进制表示的取值范围为“0－127”

  - 具有A类地址特征的网络总数为2^7(7+0)个

- B类地址

  - **前2字节**标识网络地址部分，后2字节标识主机地址部分

  - 每个网络最多可容纳 （2^16-2）台主机

  - 第1字节用十进制表示的取值范围为“128－191”

  - 具有B类地址特征的网络总数为 2^14(6+8)个

- C类地址

  - **前3字节**标识网络地址部分，后1字节标识主机地址部分

  - 每个网络最多可容纳（2^8-2）台主机

  - 第1字节用十进制表示的取值范围为“192－223”

  - 具有C类地址特征的网络总数为 2^21(5+16) 个
...
> **受限广播地址**255.255.255.255只能用作**目的地址**；
>
> **环回地址**127.\*.\*.*用来**测试**，也只能用作目的地址；
>
> 0.0.0.64可能是一个 这个网络的**特定主机**（specific host on this network）地址（即网络部分全为0的地址），被一个网络的主机用于给同一个网络的某个主机发分组，只能用作**目的地址**。 
>
> 0.0.0.0表示这个网络的这个主机，仅可用作**源IP地址**，在刚启动且还未获得IP地址时使用。
...
- 32位全为1，255.255.255.255  Flood Broadcast
  ...
  ![image-20230613150637569](/assets/images/cs_network.assets/image-20230613150637569.png)

- 主机部分全为0，如172.16.0.0  网络地址

- 主机部分全为1，如172.16.255.255  Direct Broadcast

  ![image-20230613150650673](/assets/images/cs_network.assets/image-20230613150650673.png)
...
![image-20230613150737140](/assets/images/cs_network.assets/image-20230613150737140.png)
...
![image-20230613152015621](/assets/images/cs_network.assets/image-20230613152015621.png)
...
- 子网规划将导致IP地址空间的损失
  ...
  - 如果借2位主机位创建子网，可以有**4个子网，但只有两个可用**，每个子网可容纳64个IP，但只有62个可用，总可用IP数量为，2*62=124。

    > 从网络中借位的子网，要看有没有占用了网络的网络地址和主机地址、子网的网络地址和主机地址
    
    ![image-20230613152426513](/assets/images/cs_network.assets/image-20230613152426513.png)
...
![image-20230613155405586](/assets/images/cs_network.assets/image-20230613155405586.png)
...
![image-20230613161015812](/assets/images/cs_network.assets/image-20230613161015812.png)

**IPv6基本属于**

![image-20230613161402788](/assets/images/cs_network.assets/image-20230613161402788.png)
...
![image-20230613162140736](/assets/images/cs_network.assets/image-20230613162140736.png)

![image-20230613162148107](/assets/images/cs_network.assets/image-20230613162148107.png)

- IPv6扩展头

![image-20230613162256825](/assets/images/cs_network.assets/image-20230613162256825.png)
...
- CIDR的基本思想描述在 **RFC 1519** 中
  - **分配IP地址的时候不再以类别来分（和可变长子网掩码VLSM有差别），而是按照可变长的地址块来分配**
    
    - 如：某用户需要 2000 个地址
    
      > CIDR和NAT：https://www.cnblogs.com/vamei/archive/2013/01/24/2798226.html解决地址枯竭的问题

### CIDR路由

- **路由表必须扩展**，增加一个 32-bit 的**子网掩码**
...
- 当一个分组到来到的时候
  ...
  - 如果路由表中有多个表项匹配 (这些表项有不同的子网掩码) ，使用**子网掩码最长的那个表项**
...
- PAT：port address translate（超载）
  - 将多个**私有IP地址**影射到**同一个公有IP地址的不同端口**
- Private IP address：不可路由的地址、也可用于广域网链路上

![image-20230624145225186](/assets/images/cs_network.assets/image-20230624145225186.png)

**NAT带来的问题**

- NAT**违背了IP的结构模型** –每个IP地址唯一地标识了一台机器
...
> - 终点不可达：ICMP中port不可达
> - 原点抑制：流量控制，避免拥塞
> - 超时：TTL为0，路由器丢掉报文，返回ICMP超市报文
> - 参数问题：二义性
> - 改变路由：改变最优路径

![image-20230613164302431](/assets/images/cs_network.assets/image-20230613164302431.png)

**应用 1：ping的工作原理**

- 使用**ping命令**（即调用ping过程）时，将向目的站点发送一个**ICMP回声请求报文**（包括一些任选的数据），
- 如目的站点接收到该报文，必须向源站点发回一个**ICMP回声应答报文**，源站点**收到应答报文**（且其中的任选数据与所发送的相同），则认为目的站点是**可达的**，否则为不可达。
...
- tracert过程是通过**ICMP数据报超时报文来得到一张途经的路由器列表**
...
- 接着源主机再向目的主机发第二个IP报文，并置TTL为2，然后再发第三个、第四个IP数据报，……直至到达目的主机
  ...
  ![image-20230613164747619](/assets/images/cs_network.assets/image-20230613164747619.png)
...
- 结果：MTU=900
  ...
  ![image-20230613164817125](/assets/images/cs_network.assets/image-20230613164817125.png)
...
- **免费**ARP
  ...
  - 当一台**主机启动**时，发送要给一个免费ARP，（如果意外收到一个应答，即是IP地址发生了冲突）

  - 当一个**接口（interface）的配置**发生了**改变**，会发送一个免费ARP

    > 启动时、配置改变

- **代理ARP**

  - 当源设备需要的目的地址与自己**不在同一个网络**时，如果源不知道目的MAC地址，它必须使用**路由器的服务**使它的数据达到目的，当路由器在这种方式下使用时，称为**缺省网关**。

  - **缺省网关**是与源设备所处的网段相连的路由器接口上的**IP地址**
...
- ARP表
  ...
  - 为了减少ARP请求的次数，**每个设备**拥有**自己的ARP表，包括路由器**。
  - 自动维护ARP表
...
- ARP欺骗
  ...
  ![image-20230613170705515](/assets/images/cs_network.assets/image-20230613170705515.png)
...
- 动态分配
  ...
  ![image-20230706111533879](/assets/images/cs_network.assets/image-20230706111533879.png)
...
IPv4分组结构：https://blog.csdn.net/weixin_44164489/article/details/107820407

IPv4分组由两部分组成：分组头和数据。**与TCP报文类似**，IP分组头的**基本单位也为4字节**，即分组头的每行宽度为4字节，如下图所示前5行是每个分组头必须有的字段，第6行是选项字段，因此IP分组头的**基本长度为5*4=20字节**，如果加上**最长为40字节的选项**，则IP**分组头的最大长度为60字节**。

可见IP分组头的长度为20-60字节。值得一提的是，TCP报头长度也为20-60字节

IP分组：根据MTU限制，除了最后一个分片，之前的分片必须是**8的整数倍**，先根据MTU求出链路所能承载的最大数据（8的倍数），然后忽略分组头开始计算分组

IPv6的基本头：版本、业务等级、流标记、|、净荷长度、下一个头、跳数限制、|、信源地址、|、信宿地址（共40个字节）

IPv4的私有地址：A类1个网络，B类16个，C类256个
...
*网络地址：*全0的ip地址作为当前的网络地址，这个地址可以让机器在不知道自己网络号的情况下访问自己所在的网络，但是不会作为目的网络而发送东西的。子网的网络地址不会作为分组的目的地址。

路由器隔离一个广播域（为了减少信息的传输量，同时避免网络风暴，路由器会把泛洪的分组卡下来。）也就是说，这个分组只能在所处的局域网中传播到每一台主机上，而不是整个互联网。

组播出去不需要知道源地址,不会出现在分组头部的源地址子段当中



RIP认为跳数超过15跳为不可达

两台建立了全毗邻关系的路由器，LSP数据库完全一致。

了解NAT原理和NAT映射表

ICMP报文分为差错报告报文和信息报文，封装在IP分组中，有2B校验字段

BGP协议连接不同的互联网服务商自治系统间的协议

当TTL的值等于0时，路由器会丢掉一个traceroute 分组，相当于分组走到头了，返回一个超时信号

动态路由路由选择协议的两个主要任务是网络发现和更新、维护路由表

路由和转发的区别：路由是寻找路径的过程，转发是沿路径发送分组的过程。路由是涉及整个网络的过程，转发是本地操作。

CIDR协议用于缓解地址资源枯竭，缩小路由表的规模

划分子网要区分是CIDR聚合还是有分类的VLSM，CIDR可以地址范围重叠（只要说明地址前缀长度即可，不考虑地址分类），VLSM不可以重叠（最低位适配原则）

默认路由（缺省路由）：https://blog.csdn.net/a1232345/article/details/38761819

> 优先级：静态路由>动态路由>默认路由

计算网络地址技巧：计算ip地址a位，按2^a叠加，得到一系列可用网络地址

IP分组头中的标记字段，MF=1表示后面还有分片，MF=0表示这是最后一个分片

定向广播地址就是子网广播地址，受限广播地址就是255.255.255.255>https://zhuanlan.zhihu.com/p/598430656

翻译/转换技术是直接转换IPv4/v6地址；双栈技术是既采用IPv4又采用IPv6；只有隧道技术，是IPv6分组整体被当作数据封装到v4分组中。
...
![image-20230611162157722](/assets/images/cs_network.assets/image-20230611162157722.png)
...
![image-20230611160940691](/assets/images/cs_network.assets/image-20230611160940691.png)
...
- **传输层原语独立于网络层原语**，而网络层原语会因为网络的不同而不同（同一接口）
...
![image-20230611162122134](/assets/images/cs_network.assets/image-20230611162122134.png)



作用范围比较

![image-20230611163522618](/assets/images/cs_network.assets/image-20230611163522618.png)
...
- <1023 : 用于公共应用（保留，全局分配，用于标准服务器），IANA分配；知名端口号
...
![image-20230625212021432](/assets/images/cs_network.assets/image-20230625212021432.png)
...
- 校验的范围包括头部、数据（有效载荷）和概念性的伪头部

![image-20230611191501582](/assets/images/cs_network.assets/image-20230611191501582.png)

> TCP报头：源端口、目的端口|Seq num|Ack num|TCP报头长度、（urg、ack、psh、syn、fin）、接收方窗口大小|校验和、
...
![image-20230611191715461](/assets/images/cs_network.assets/image-20230611191715461.png)



**重复连接请求CR**

![image-20230611191928898](/assets/images/cs_network.assets/image-20230611191928898.png)
...
为了避免两军队（two-army）问题，使用**定时器**

- **如果一方发送了FIN数据段**出去却在一个设定的时间没有收到应答，释放连接
- **另一方最终会注意到连接的对方已经不在了**，超时后连接释放

![image-20230611192636516](/assets/images/cs_network.assets/image-20230611192636516.png)
...
![image-20230611194128889](/assets/images/cs_network.assets/image-20230611194128889.png)

>窗口尺寸受制于接收方，接收方通知发送方窗口剩多少，并确认收到上一个分组
...
![image-20230611194612208](/assets/images/cs_network.assets/image-20230611194612208.png)
...
![image-20230611194729702](/assets/images/cs_network.assets/image-20230611194729702.png)

> 一批一批发送

**傻瓜窗口综合症** 

另一个使TCP性能退化的问题是傻瓜窗口综合症（silly window syndrome problem）：当有**大块数据被传递**给发送端TCP实体，但接收端的交互式应用每次**只读取一个字节**的时候，问题就来了
...
![image-20230611195130211](/assets/images/cs_network.assets/image-20230611195130211.png)
...
> 总结一下，接收方的优化是一批批发送、等待积累足够的窗口才通知发送方
...
- 所有的互联网TCP算法都假定超时是由拥塞引起的，并且通过**监视超时**的情况来判断是否出现问题
...
> 拥塞检测通过监视超时，拥塞控制通过维护窗口（接收窗口和拥塞窗口中的最小值），主要是拥塞窗口的大小
...
除了使用**接收者窗口和拥塞窗口**，TCP拥塞控制还是用了第三个参数，**阈值（threshold）**，初始化为64K
...
![image-20230611202934346](/assets/images/cs_network.assets/image-20230611202934346.png)
...
![image-20230625192643277](/assets/images/cs_network.assets/image-20230625192643277.png)

> 三个参数：接收窗口、拥塞窗口、阈值
>
> 过了慢启动阈值后，拥塞避免算法可以避免窗口增长过快导致窗口拥塞，而是缓慢的增加调整到网络的最佳值。
>
> 慢启动、拥塞避免、拥塞处理（快重传、快恢复）
>
> 横坐标是一次RTT，纵坐标是一次拥塞窗口
...
- **保活定时器**（keep-alive timer）用来**检查连接是否存活**，当一个连接空闲的时间超过保活定时器的时间，该连接将被杀掉。
- 最后一个定时器是在关闭时刻处于**时间等待定时器TIMED WAIT** 状态中使用的定时器，它运行两倍的最大分组生存时间，以确保连接关闭之后，该连接上的所有分组都完全消失
...
**UDP实体**产生的数据段，从源端（end point）发送到 目的端点（end point），**端点由IP和Port**两个元素指定，实际上端点唯一绑定了某个**应用进程**，所以，UDP数据段是**从端到端的，即进程到进程的**，UDP中的端口信息则可以让目标机器根据端口选择**正确的处理程序**。

**源端口和目的端口**是UDP数据段中最重要的两个字段，**校验和**也是**UDP**的一个字段

0~65535的端口号都是合法的值

**知名端口号（0-1023）**通常用于知名应用的服务端程序，而客户端程序通常使用**临时端口号**，或成**动态端口号，49152~65535**



**SYN**数据段不是标准的数据段，而是三次握手建立连接信息中的**第一次、第二次握手信息**，其中的**SYN控制位置位（SYN=1）**，**第三次握手信息**是**标准数据段，SYN=0**

**确认号字段ACK**的值表明了**期待接收的第一个数据字节的序列号**（**已知初始序号**的情况下，此序列号**之前**编号的那些字节已经被正确接收了）；而**窗口尺寸WIN**意谓着接收方能够接收的数据段的长度上限值。**序列号Seq**表明数据段中第一个字节的编号。

**累积的ACK（拥塞控制）** 数量不严格单调递增。如果中间有丢包现象，两次累计确认的值应该相当，即x=y。比如连续到达的两个段是x和x+s，当两个连续的段到达，那么会送的ack应该是x、x+s=y。

>1 下列哪些关于TCP 报头的陈述不准确?
>A、TCP 段头不包含数据段总长度有关的信息（TCP头长度）
>B、TCP段头允许接收端标记到目前为止接收了什么数据和可达的缓冲空间（ack和win不能说明收到的数据）
>C、TCP 段头包含确定TCP 数据段的源和目的地的数据
>D、TCP段头有校验位,用来保护有效载荷（还有头部、伪头部）
>E、TCP 段头被用来确定数据段应被传递到的套接字（段中只有端口号，缺少IP地址）
>
>正确答案： BDE



**快速重传**是TCP假设**三个重复确认（不包括第一次ACK）**意味着已经丢失一个包，**启动重传**，而不必等到重传定时器超期。

发送窗口=Min{接收窗口，拥塞窗口}

> 重传定时器：分组丢失 
>
> 持续定时器：避免死锁，处理零窗口大小通告
>
> 保活定时器：检查连接
>
> 时间等待定时器：等连接关闭后分组都消失

持续定时器和保活定时器都会发送**probe探测数据段**，但是保活定时器超时的probe是服务器端TCP实体发出的，只有**持续定时器**超时的probe是**数据发送方TCP实体**发出的。



1、**最小TCP MTU**的总长度是多少？包括TCP和IP的开销，但是不包括数据链路层的开销。

TCP默认**有效载荷为536**个字节，**TCP头开销为20个字节，IP头开销为20个字节**。因此最小TCP MTU的总长度是**576**个字节。

2、TCP段的最大有效载荷为65495字节，为什么会选择这样奇怪的数值？

**TCP被装载于IP的有效载荷中**，IP的有效载荷为65515个字节，而TCP头为20个字节。因此TCP的有效载荷为65515-20=65495字节。

三次握手建立TCP连接；而释放TCP连接相当于释放两根单向连接，**FIN+ACK一来一回**释放一个方向的连接；**再进行FIN+ACK**释放反方向的连接，此时TCP连接才算关闭，所以，释放连接需要**4次握手**。

**TCP实体的状态ESTABLISED**状态是建立好了连接之后的状态，可以在此状态下收发数据



‌拥塞控制：考虑一个RTT 为 20 ms的 TCP 连接。假设网络容量是 10 Mbps ，数据段长是 1000 Bytes。如果要充分利用网络容量，所需的滑动窗口大小是200kb。
...
![image-20230615113803069](/assets/images/cs_network.assets/image-20230615113803069.png)
...
- 域名
  ...
  - 各组成部分的名字**最多有 63 个字符长**，整个路径**不超过 255 个字符**
  - 没有规则限制同时在两个或多个顶级域名下的注册 (如：sony.com and sony.nl)—域名抢注
  - 每个域自己控制它下面的域（子域）的划分
  - 要创建一个新的域，创建者必须得到**该新域的上级域的许可**，一旦创建成功，该新域可以创建子域，而无需征得上级域的同意
...
- 资源记录
  ...
  - 当一个解析器把域名传递给DNS时，DNS所返回的是与该域名相关联的资源记录。所以**DNS的主要功能是将域名映射到资源记录**上
  ...
  - 一个资源记录包括5个部分：
    ...
    - 类型（Type）
      ...
      ![image-20230626143751955](/assets/images/cs_network.assets/image-20230626143751955.png)
...
- 域名解析
  ...
  - **如果待查询的域名落在该名字服务器的管辖范围内**，它将返回权威资源记录
    ...
    - 一个权威资源记录（authoritative record）是指来自于管理该记录的权威机构，因此总是正确的，它**和缓存的记录不同**，后者可能是过期的
  ...
  - 域名解析的种类
    ...
    > 主机向本地域名服务器递归查询，本地域名服务器向根域名服务器迭代查询
    >
    > 权威资源记录：域名在该服务器的管辖范围内，和缓存记录不同，永远正确

    ![image-20230615120132966](/assets/images/cs_network.assets/image-20230615120132966.png)
...
- 电子邮件体系结构
  ...
  ![image-20230615120411820](/assets/images/cs_network.assets/image-20230615120411820.png)
  ...
  - SMPT – Simple Mail Transfer Protocol
    - **源机与目标机**（SMTP守护进程在监听）的25端口建立TCP连接
      ...
      - 数据交换
        ...
        - 客户发送消息，服务器回发确认
      - 连接释放
    - 如果消息不能被投递，则向消息的发送方返回一个**错误报告**（包含了不能投递消息的第一部分）
    - 这是一个简单的 **ASCII 协议**
  
  > STMP:源机和目标机建立TCP通行，简单的ASCII协议
  >
  > POP3：最后传递
  
  - 最后传递
    - 当有用户使用拨号上网，不可能一直在线的时候，上述工作模式不再运作了
    - 一种解决方案是在ISP的一台机器上运行一个**消息传输代理（message transfer agent）；** 这台机器可以一天24小时运行，
...
- 一种可**靠的面向连接**的服务，采用TCP在支持FTP的系统间传输文件，它支持**双向二进制文件和ASCII文件传输**。
- TFTP：一种无连接的不可靠的服务，采用**UDP**在支持TFTP的系统间传输文件。
- FTP ：使用**两条 TCP链接**

> 文件传输：TFTP基于UDP，FTP基于两条TCP连接，双向二进制文件和ASCII文件传输
...
![image-20230615124014715](/assets/images/cs_network.assets/image-20230615124014715.png)
...
递归解析：向本地域名服务器询问，等待DNS服务端的全部解析结果（别人帮忙全部做完）

迭代解析：本地域名服务器只给出下一级域名服务器的地址，由DNS客户端自己继续查询（自己全部做完）

一个zone通常有两台域名服务器，主域名服务器负责创建、维护和更新zone数据；而备份服务器只从主域名服务器获取数据。

resolver就是DNS客户，负责发出解析请求和接收应答。

DNS在TCP和UTP上运行



UA的主要功能**是收、发邮件，提供读、写邮件（消息）**的功能，邮件包括信封和消息，而消息又包括头部（header）和主体（body）两部分。

> 邮件-信封、消息-头部、主体

MIME允许传送七大类数据，包括文本、Multipart、消息、图像、视频、音频、应用。
...
为了缓解前端的压力，一种解决的方法是TCP移交，TCP端点被传递给处理节点 ，所以应答可以直接向客户端发送 
